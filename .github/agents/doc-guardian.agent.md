...
3. **Configure upstream remote and push to rdkcentral** (CRITICAL - follow these exact steps):
   ```bash
   # Add rdkcentral as upstream remote if not already present
   git remote add upstream https://github.com/rdkcentral/entservices-usersettings.git || true
   
   # Fetch upstream to ensure we have the latest develop branch
   git fetch upstream
   
   # Push your documentation branch to rdkcentral upstream
   git push upstream HEAD:doc-update/<source-branch>-docs
   ```
   
4. **Create PR against rdkcentral using gh CLI**:
   ```bash
   # Create PR with rdkcentral as base repository
   gh pr create \
     --repo rdkcentral/entservices-usersettings \
     --base develop \
     --head doc-update/<source-branch>-docs \
     --title "[DOCS] Documentation updates for PR #<original-pr>" \
     --body "## Documentation Updates

   This PR contains documentation updates for melhar098/entservices-usersettings#<original-pr>

   ### Files Updated
   - List documentation files updated here

   ### Summary of Changes
   - Describe changes made

   ### Link to Original PR
   melhar098/entservices-usersettings#<original-pr>

   ### Areas Needing SME Review
   - Note any areas needing review

   Doc-Guardian-Issue: #<this_issue_number>"
   ```
   
   **IMPORTANT**: The base repository MUST be `rdkcentral/entservices-usersettings` with base branch `develop`. Do NOT create a PR against the fork (melhar098).

5. Set documentation PR as **dependent on original PR** or reference it
