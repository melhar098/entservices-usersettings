...
5. Configure credentials and push commits to the existing Copilot PR branch:
   - **Prerequisite**: Configure a **Copilot Cloud agent secret** named `FORK_PAT` with read/write access to both the fork repository AND the upstream repository.
   - **Token scope**: Since you have contributor access to rdkcentral, scope `FORK_PAT` to both:
     - Your fork (e.g., `melhar098/entservices-usersettings`)
     - Upstream repo (e.g., `rdkcentral/entservices-usersettings`)
...
6. Use the same token for PR creation:
   - Export `FORK_PAT` for GitHub CLI:
     ```bash
     export GH_TOKEN="$FORK_PAT"
     export GITHUB_TOKEN="$FORK_PAT"
     ```
   - Verify upstream access: `gh repo view "$UPSTREAM_REPO" --json nameWithOwner,viewerPermission`
   - If viewerPermission is `READ`, `NONE`, or the command fails with 403/404, report insufficient permissions and provide manual PR instructions.
7. Create a cross-fork PR targeting the `develop` branch of the **upstream** repo from the same `CURRENT_BRANCH` already used by the Copilot PR:
   ...
   - If `gh pr create` exits with an error (e.g. 403 Forbidden, "Resource not accessible by integration", GraphQL error, or similar), capture the error message and post a comment on the originating issue explaining:
     ...
     - That this may indicate `FORK_PAT` does not have write access to the upstream repository, or the token needs to be scoped to both fork and upstream repos.
...
8. Set documentation PR as **dependent on original PR** (if available) and link back to the tracking issue.
